...
public class TexGround{
    ...
    private final String vertexShaderCode =             //위치 vertex 마다 실행
            "attribute vec4 vPosition;" +               //attribute는 vertex 마다 전달 돠는(좌표,색상,법선백터,택스트 좌표)
                    //"attribute vec4 vColor;"+ //꼭짓점마다 안바뀜
                    "uniform mat4 MVP;"+
                    //"varying vec4 fColor;"+
                    //"uniform float theta;"+
    ...
    public TexGround(MainGLRenderer renderer) {
    ...
    public void draw(float[] mtxProj, float[] mtxView, float[] mtxModel) {
        ...
        //GLES20.glVertexAttribPointer(colorHandle, COORDS_PER_VERTEX, GLES20.GL_FLOAT, false, vertexStride, colorBuffer);
